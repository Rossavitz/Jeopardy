...
async function getCategory(idArray) {
  ...
  idArray.forEach(async function (id) {
    //loop through each ID to get response.
    let response = await axios.get(
...
//create the table, append the table, create and add head then body, append.
function fillTable() {
...
async function setupAndStart() {
  //function to access apis in order.
  await getCategoryIds();
  await getCategory(idArray);
  setTimeout(fillTable, 100); //fillTable would run before waiting for APIs, so set small timeout to make sure data was retrieved. (await does not work)
}
